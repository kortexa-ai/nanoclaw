...
import { detectRuntimeMode } from './ssh-runtime.js';
import {
  getAllChats,
...
async function runAgent(
  ...
  try {
    ...
    if (isSshMode) {
      ...
      if (!node) {
        logger.error({ group: group.name }, 'No available SSH nodes for agent');
        return 'error';
      }
      // SSH: inject stdin-based IPC handlers
      const onSshProcessRegistered = (proc: ChildProcess, containerName: string) => {
        ...
        queue.registerProcess(chatJid, proc, containerName, group.folder, ipcHandlers);
      };
      const onIpc = async (payload: IpcPayload) => {
        await processIpcPayload(payload, group.folder, isMain, ipcDeps);
      };
      output = await runSshAgent(group, containerInput, onSshProcessRegistered, wrappedOnOutput, node, onIpc);
    } else {
      // Docker: default file-based IPC (no handlers needed)
      const onProcessRegistered = (proc: ChildProcess, containerName: string) =>
        queue.registerProcess(chatJid, proc, containerName, group.folder);
      output = await runContainerAgent(group, containerInput, onProcessRegistered, wrappedOnOutput);
    }
...
async function main(): Promise<void> {
  ...
  // MQTT is always on — local communication channel
  const mqtt = new MqttChannel(MQTT_PORT, channelOpts);
  channels.push(mqtt);
  await mqtt.connect();

  // Auto-register mqtt:local as main group if no main group exists
  const hasMain = Object.values(registeredGroups).some(g => g.folder === MAIN_GROUP_FOLDER);
  if (!hasMain) {
    registerGroup('mqtt:local', {
      name: 'MQTT Local',
    ...
    logger.info('Auto-registered mqtt:local as main group');
  }

  // Build shared IPC deps (used by both file watcher and SSH stdout relay)
  ipcDeps = {
  ...
  startSchedulerLoop({
    ...
    queue,
    onProcess: (groupJid, proc, containerName, groupFolder, ipcHandlers) => {
      queue.registerProcess(groupJid, proc, containerName, groupFolder, ipcHandlers);
    },
